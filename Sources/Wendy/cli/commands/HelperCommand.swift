...
struct HelperCommand: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "helper",
        abstract: "Manage the Wendy USB device monitoring helper daemon",
        discussion: """
...
extension HelperCommand {
    ...
    struct Install: AsyncParsableCommand {
        ...
        func run() async throws {
            let logger = Logger(label: "sh.wendy-helper-install")

            logger.info("Installing Wendy Helper Daemon...")
        ...
        private func installNetworkDaemon() async throws {
                ...
                if #available(macOS 13.0, *) {
                    let service = SMAppService.daemon(
                        plistName: "sh.wendy.wendy-network-daemon.plist"
                    )
    ...
    struct Uninstall: AsyncParsableCommand {
        ...
        func run() async throws {
            let logger = Logger(label: "sh.wendy-helper-uninstall")

            logger.info("Uninstalling Wendy Helper Daemon...")
    ...
    struct Status: AsyncParsableCommand {
        ...
        func run() async throws {
            let plistPath = getLaunchdPlistPath()
            let serviceName = "sh.wendy.helper"
            ...
            if isInstalled {
                ...
                if isLoaded {
                    print("📝 To view logs: wendy helper logs")
                }
            } else {
                print("💡 To install: wendy helper install")
            }
        }
    ...
    struct Logs: AsyncParsableCommand {
        ...
        func run() async throws {
            let logPath = "\(NSHomeDirectory())/Library/Logs/wendy-helper.log"
...
private func getLaunchdPlistPath() -> String {
    return "\(NSHomeDirectory())/Library/LaunchAgents/sh.wendy.helper.plist"
}

private func getHelperExecutablePath() throws -> String {
    // Try to find the wendy-helper executable
    let possiblePaths = [
        "\(NSHomeDirectory())/bin/wendy-helper",
        "/usr/local/bin/wendy-helper",
        "/opt/homebrew/bin/wendy-helper",
        "./wendy-helper",
        "./.build/debug/wendy-helper",
        "./.build/release/wendy-helper",
    ]
...
private func generateLaunchdPlist(
    ...
    let logPath = logFile ?? "\(NSHomeDirectory())/Library/Logs/wendy-helper.log"
