...
struct InitCommand: AsyncParsableCommand {
    ...
    private func initializePythonProject() async throws {
        // Create app.py
        let appPyPath = projectPath.hasSuffix("/") ? "\(projectPath)app.py" : "\(projectPath)/app.py"
        let appPyContent = #"""
        #!/usr/bin/env python3
        """
        ...
        do {
            try appPyContent.write(to: URL(fileURLWithPath: appPyPath), atomically: true, encoding: .utf8)
        } catch {
            throw InitError.fileCreationFailed(path: appPyPath, error: error.localizedDescription)
        }

        // Create requirements.txt
        let requirementsPath = projectPath.hasSuffix("/") ? "\(projectPath)requirements.txt" : "\(projectPath)/requirements.txt"
        let requirementsContent = "debugpy\n"
        do {
            try requirementsContent.write(to: URL(fileURLWithPath: requirementsPath), atomically: true, encoding: .utf8)
        } catch {
            throw InitError.fileCreationFailed(path: requirementsPath, error: error.localizedDescription)
        }

        // Create Dockerfile
        let dockerfilePath = projectPath.hasSuffix("/") ? "\(projectPath)Dockerfile" : "\(projectPath)/Dockerfile"
        ...
        """#
        do {
            try dockerfileContent.write(to: URL(fileURLWithPath: dockerfilePath), atomically: true, encoding: .utf8)
        } catch {
            throw InitError.fileCreationFailed(path: dockerfilePath, error: error.localizedDescription)
        }

        // Create entrypoint.sh
        let entrypointPath = projectPath.hasSuffix("/") ? "\(projectPath)entrypoint.sh" : "\(projectPath)/entrypoint.sh"
        ...
        """
        do {
            try entrypointContent.write(to: URL(fileURLWithPath: entrypointPath), atomically: true, encoding: .utf8)
            ...
            throw InitError.fileCreationFailed(path: entrypointPath, error: error.localizedDescription)
        }
    }

    private func createDefaultWendyJson(in projectPath: String, language: ProjectLanguage) async throws {
        let fileManager = FileManager.default
        let wendyJsonPath =
        ...
        do {
            ...
            logger.debug(
                ...
                metadata: ["appId": .string(appId), "version": .string("0.0.1"), "language": .string(language.rawValue)]
            )
        } catch {
