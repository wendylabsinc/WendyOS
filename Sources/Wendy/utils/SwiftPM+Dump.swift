...
enum Serialization {
    ...
    struct Product: Codable {
        enum ProductType: Codable {
            ...
            enum LibraryCodingKeys: String, CodingKey {
                case type
            }
            ...
            func encode(to encoder: any Encoder) throws {
                ...
                case .library(let type):
                    var container = container.nestedUnkeyedContainer(forKey: .library)
                    switch type {
                    case .automatic:
                        try container.encode(LibraryType.automatic)
                    case .dynamic:
                        try container.encode(LibraryType.dynamic)
                    case .static:
                        try container.encode(LibraryType.static)
                    }
                case .plugin:
            ...
            init(from decoder: Decoder) throws {
                ...
                if container.contains(.executable) {
                    ...
                    var libraryContainer = try container.nestedUnkeyedContainer(forKey: .library)
                    let type = try libraryContainer.decode(LibraryType.self)
                    self = .library(type: type)
