...
// enrolled identity, plus a closeFn the caller must invoke when the pass is
// done. Production dials the ingest endpoint over mutual TLS; tests inject an
// in-process client.
...
// crash-safe. It presents the device's own enrolled certificate, the one
// provisioning obtained and persisted, and holds no certificate of its own.
type DataTransferWorker struct {
	...
	// disabled and Run says so once. Enrollment still provides the device
	// identity and client certificate; this is only the destination.
	ingestHost string
	// identityLogged makes the "no identity" report a once-per-process event.
	// Without it a device that is enrolled with Cloud but has no certificate
	// on disk would log the same line on every pass, for ever.
	identityLogged bool
...
// NewDataTransferWorker builds a worker that reads the enrolled device
// identity from the ProvisioningService at run time, dialling the ingest
// endpoint over mutual TLS once per pass (mirroring CloudFlusher).
...
//
// Identity is the enrolled device certificate, presented in the TLS handshake
...
// normalizeEndpoint reduces an endpoint value to the host[:port] form the
// cloud dialler expects, tolerating URL-shaped input.
...
// errNoIdentity is returned when the agent holds no enrolled device identity.
// It is not a transient dial failure and no retry can clear it: the device has
// to finish provisioning first.
var errNoIdentity = errors.New("data transfer worker: the agent holds no enrolled device identity")

// ingestIdentity returns the client certificate the worker presents to the
// ingest endpoint.
//
// There is exactly one identity and there is no fallback. It is the leaf,
// chain and key that provisioning persists: StartACMEProvisioning enrols the
// device against pki-core over Automatic Certificate Management Environment
// (ACME) with an External Account Binding and hands the result to
// persistProvisioning, which writes device.pem, ca.pem and device-key.pem and
// holds the same triple in memory; ProvisioningCerts is the read side. The
// legacy Cloud enrolment path fills the same three fields, so a device on
// either path presents whatever certificate it actually enrolled with and the
// worker never chooses between two identities.
//
// The caller owns keyData and must zero it.
func (w *DataTransferWorker) ingestIdentity() (certPEM, chainPEM string, keyData []byte, err error) {
	certPEM, chainPEM, keyData = w.provisioningSvc.ProvisioningCerts()
	if certPEM == "" || len(keyData) == 0 {
		clear(keyData)
		return "", "", nil, errNoIdentity
	}
	return certPEM, chainPEM, keyData, nil
}
...
func (w *DataTransferWorker) dialFactory(ctx context.Context) (cloudpb.DataIngestServiceClient, func(), error) {
	...
	if err != nil {
		if !w.identityLogged {
			w.identityLogged = true
			w.logger.Error("data transfer worker: no enrolled device identity; " +
				"episode uploads will not be attempted until the device is provisioned")
		}
		return nil, nil, err
	}
	...
	conn, err := func() (*grpc.ClientConn, error) {
		defer clear(keyData)
		return cloudrelay.DialCloud(w.ingestHost, certPEM, chainPEM, keyData)
	}()
	if err != nil {
		return nil, nil, err
	}
	w.identityLogged = false
	w.logger.Debug("data transfer worker: dialled ingest", zap.String("host", w.ingestHost))
