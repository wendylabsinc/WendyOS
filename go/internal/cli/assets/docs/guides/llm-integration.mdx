...
For a terminal assistant built into Wendy, run:

```sh
wendy chat
```

Wendy guides you through choosing a local server such as Ollama or LM Studio,
or a hosted provider, and selecting an available model. It remembers your
selection; use `wendy chat --setup` to change it. See
[Wendy Chat](/docs/guides/chat) for local model setup and chat controls.
