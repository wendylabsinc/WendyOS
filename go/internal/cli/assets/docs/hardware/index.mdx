...
| AMD GPU | `gpu` | ROCm compute on supported x86 Linux hosts |
| Web servers or network APIs | `network` | Serving HTTP, WebSockets, or accepting inbound connections |

<Callout type="info">
  For AMD GPU setup and a computation check, follow [Run PyTorch on an AMD GPU](/docs/guides/tutorials/python/amd-rocm-pytorch). See [GPU Access](/docs/hardware/gpu) for host and container requirements.
</Callout>
