...
from g1_sim.lidar import DIRECTIONS, HORIZONTAL_RING, Lidar, RAY_COUNT
from g1_sim.sensors import LIDAR_POSITION, SCAN_COUNT, SCAN_MAX, SCAN_MIN, PhysicsSampler, instrumented_model
...
def test_elevations_hit_actual_floor_walls_and_leave_open_sky_empty(world):
    ...
    # The G1 waist is taller: forward +15/+30deg rays clear the 2m wall.
    assert 3 * SCAN_COUNT + 180 not in result["ray_indices"]
...
def test_translation_changes_wall_and_obstacle_ranges_causally(world):
    ...
    assert before["ranges"][180] == pytest.approx(5.9 - LIDAR_POSITION[0], abs=1e-6)
...
def test_lidar_site_extrinsics_rotate_points_and_rays_together(world):
    ...
    np.testing.assert_allclose(origin, [0.0, LIDAR_POSITION[0], sim.data.qpos[2] + LIDAR_POSITION[2]], atol=1e-12)
    forward = point_for(result, HORIZONTAL_RING * SCAN_COUNT + 180)
    np.testing.assert_allclose(origin + rotation @ forward, [0.0, 5.9, origin[2]], atol=1e-6)
    np.testing.assert_array_equal(result["ranges"], sampler.scan())
...
def test_returns_inside_the_minimum_range_are_unknown(world):
    sim, sampler, runtime = world
    sim.data.qpos[0] = 5.88 - LIDAR_POSITION[0]  # Only 2cm from east wall.
    sampler.capture(runtime)
    result = Lidar().sample(sampler)
