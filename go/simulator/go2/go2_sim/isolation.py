...
TABLE = "wendy_go2"
OWNER = "Wendy managed Go2 UDP DDS isolation v1"
RULESET = '''table inet wendy_go2 {
  comment "Wendy managed Go2 UDP DDS isolation v1"
  chain input {
...
def install():
    ...
    if existing:
        ...
        if len(lines) < 2 or lines[0] != "table inet wendy_go2 {" or lines[1] != f'comment "{OWNER}"':
            raise RuntimeError("reserved nftables table has an unknown owner; refusing to replace it")
        prefix = "delete table inet wendy_go2\n"
