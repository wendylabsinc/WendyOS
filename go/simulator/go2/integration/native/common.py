...
DEFAULT_Q = [0.1, 0.8, -1.5, -0.1, 0.8, -1.5, 0.1, 1.0, -1.5, -0.1, 1.0, -1.5]
...
class SimulatorAPI:
    ...
    def status(self):
        ...
        require(value.get("simulation") is True and value.get("robot") == "go2"
                ...
                "endpoint did not identify itself as the Wendy Go2 simulation")
        if self.vm_name is not None:
            require(value.get("vm_name") == self.vm_name and value.get("robot_kind") == "go2"
                    and value.get("dds_isolation") == "udp-rtps-loopback"
                    and value.get("clock_mode") == "device"
                    and value.get("policy_bundle") == "go2-moe-cts-164k-0.6715-v1",
                    "endpoint does not match the named managed Go2 VM and its DDS isolation profile")
