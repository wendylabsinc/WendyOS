...
def identity(**changes):
    return {"simulation": True, "robot": "go2", "profile_version": 1, "robot_kind": "go2",
            "vm_name": "go2-sim", "dds_isolation": "udp-rtps-loopback", "clock_mode": "device",
            "policy_bundle": "go2-moe-cts-164k-0.6715-v1", "source_digest": "sha256:" + "a" * 64,
            **changes}
...
class EnvironmentTests(unittest.TestCase):
    ...
    def test_missing_mismatched_or_unmanaged_identity_is_rejected(self):
        for changes in ({"vm_name": "another-vm"}, {"dds_isolation": "unmanaged"},
                        {"robot_kind": "g1"}, {"source_digest": ""}, {"source_digest": None},
    ...
    def test_explicit_source_pin_and_loopback_url_are_enforced(self):
        api = self.api(vm_name="go2-sim", source_digest="sha256:" + "b" * 64)
        ...
        for url in ("http://192.168.123.161:8890", "http://127.0.0.1:8890/redirect", "https://127.0.0.1"):
            with self.assertRaises(AssertionError):
                SimulatorAPI(url, vm_name="go2-sim")
