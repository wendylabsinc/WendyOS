...
def main() -> None:
    ...
    mode = environment.get("G1_RUNTIME_MODE", "inference")
    package = environment.get("G1_RUNTIME_PACKAGE", "runtime")
    if not package.replace("_", "").isalnum():
        raise RuntimeError("invalid G1_RUNTIME_PACKAGE")
    module = {
        "inference": f"{package}.inference_service",
    }.get(mode)
    if module is None:
