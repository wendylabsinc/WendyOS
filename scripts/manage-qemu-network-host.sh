...
check_port_available() {
    ...
    if sudo ss -ulnp | grep -q ":53.*${bridge_ip}"
    then
...
start_dnsmasq() {
    ...
    then
        ...
        if echo "${error_output}" | grep -q "port 53"
        then
        ...
        elif echo "${error_output}" | grep -q "bind"
        then
...
show_status() {
    ...
    if [[ "${bridge_up}" == "true" ]] && [[ "${tap_up}" == "true" ]] && [[ "${dnsmasq_running}" == "true" ]] && [[ "${ip_forward}" == "true" ]]
    then
...
restore_ip_forward_state() {
    ...
    then
        ...
        if execute "sudo sysctl -w net.ipv4.ip_forward=${original_state}"
        then
...
cleanup_network() {
    ...
    then
        ...
        # First, bring it down
        if ! execute "sudo ip link set '${BRIDGE}' down 2>/dev/null"
        then
            warning "Failed to bring down bridge (may already be down)"
        fi

        # Remove the bridge
        if ! execute "sudo ip link del '${BRIDGE}' 2>/dev/null"
        then
    ...
    then
        ...
        if ! execute "sudo ip link del '${TAP}' 2>/dev/null"
        then
...
setup_network() {
    ...
    if [[ "${setup_failed}" == "false" ]] && ! create_bridge
    then
        error "Failed to create bridge"
    ...
    if [[ "${setup_failed}" == "false" ]] && ! start_dnsmasq
    then
        error "Failed to start dnsmasq"
